...
if [ -f "$LOG_AUTH" ]; then
    # Look for specific authentication failures (more precise)
    FAILED_LOGINS=$(grep -Ei "Failed password|authentication failure|Invalid user" "$LOG_AUTH" | tail -n "$MAX_LINES")
    
    if [ -z "$FAILED_LOGINS" ]; then
        echo -e "${GREEN}  No failed login attempts detected.${NC}"
    else
        log_warn "Recent failed attempts detected:"
        # Print full line with timestamp for context
...
if [ -f "$LOG_AUTH" ]; then
    SUDO_CMDS=$(grep "COMMAND=" "$LOG_AUTH" 2>/dev/null | tail -n 5)
    
    if [ -z "$SUDO_CMDS" ]; then
        echo -e "${GREEN}  No recent sudo commands found.${NC}"
    else
        echo "$SUDO_CMDS" | awk -F'COMMAND=' '{print "  → " $2}'
    fi
else
    log_info "Auth log not found. Skipping..."
fi
...
if [ -f "$LOG_DPKG" ]; then
    ...
    if [ -z "$CHANGES" ]; then
        ...
        # More flexible parsing - print full relevant info
        echo "$CHANGES" | awk '{print "  → " $1, $2, ":", $3, $4, $5}'
    fi
else
